"use client"
import React, { useEffect, useRef, useState, createContext, useContext, useCallback, useMemo, memo } from "react"
...
export const Carousel = memo(({ items, initialScroll = 0 }) => {
  ...
  return (
    <CarouselContext.Provider value={contextValue}>
      <div className="relative w-full ">
        ...
        >
          <div className={cn("absolute right-0  h-auto w-[5%] overflow-hidden bg-gradient-to-l")}></div>
          ...
          <div className={cn("flex flex-row justify-start gap-4 pl-4", "mx-auto max-w-7xl")}>
            {items.map((item, index) => (
              <motion.div
                ...
                key={"card" + index}
                className="rounded-3xl last:pr-[5%] md:last:pr-[33%] border-gray-400/100 cursor-pointer max-sm:w-[400px] "
              >
                {item}
              </motion.div>
            ))}
        ...
        <div className="mr-10 flex justify-end gap-2">
          <button
            className="relative  flex h-10 w-10 items-center justify-center bg-cyan-400 disabled:opacity-50"
            onClick={scrollLeft}
...
export const Card = memo(({ card, index, layout = false }) => {
        ...
        {open && (
          <div className="fixed inset-0 z-50 h-screen overflow-auto ">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 h-full w-full bg-[#0E0C15] backdrop-blur-lg"
            />
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
...
export const BlurImage = memo(({ height, width, src, className, alt, ...rest }) => {
